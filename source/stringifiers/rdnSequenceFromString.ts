...
import {
    _encode_UnboundedDirectoryString as _encode_UBS,
...
export
const nameToAttribute = new Map<string, [ OBJECT_IDENTIFIER, (str: string) => ASN1Element ]>([
...
function atavFromString (
    str: string,
    encodingMap: Map<string, [ OBJECT_IDENTIFIER, (str: string) => ASN1Element ]> = nameToAttribute,
): ATAV {
    ...
    if (attr) {
        ...
        if (oidRegex.test(name)) { // Then it is just an OID.
            const oid: OBJECT_IDENTIFIER = new ObjectIdentifier(str.split(".").map((c) => Number.parseInt(c)));
            return new ATAV(
                oid,
    ...
    for (let i: number = 0; i < str.length; i++) {
        ...
        if ((char === atavDelimiter) && (prevChar !== escape)) {
            const escapedATAVString = str
                .slice(start, i)
                .split(escapeString)
                .map((segment: string): string => (segment.length) ? segment : escapeString)
                .join("");
            yield atavFromString(
                escapedATAVString,
                encodingMap,
            );
            i++;
...
function* rdnSequenceFromString (
    ...
    for (let i: number = 0; i < str.length; i++) {
        ...
        if ((char === rdnDelimiter) && (prevChar !== escape)) {
            ...
            yield Array.from(rdnFromString(
                unescapedRDN,
                escape,
                atavDelimiter,
                encodingMap,
