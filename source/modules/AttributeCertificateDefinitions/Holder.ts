...
class Holder {
    ...
    public static fromElement (value: ASN1Element): Holder {
        ...
        if (holderElements.length < 1) {
            throw new errors.X500Error(`Invalid number of elements in Holder: ${holderElements.length}.`);
        }
        if (!DERElement.isInCanonicalOrder(holderElements)) {
            throw new errors.X500Error("Elements of Holder were not in canonical order.");
        }
            ...
            if (he.tagClass === ASN1TagClass.context && he.construction === ASN1Construction.constructed) {
                if (he.tagNumber === 0) {
                    if (baseCertificateID) {
                        throw new errors.X500Error("Holder.baseCertificateID already defined.");
                    }
                    baseCertificateID = IssuerSerial.fromElement(he);
                } else if (he.tagNumber === 1) {
                    if (entityName) {
                        throw new errors.X500Error("Holder.entityName already defined.");
                    }
                    entityName = he.sequence;
                } else if (he.tagNumber === 2) {
                    if (objectDigestInfo) {
                        throw new errors.X500Error("Holder.objectDigestInfo already defined.");
                    }
