...
class BasicConstraintsSyntax {
    ...
    public static fromElement (value: ASN1Element): BasicConstraintsSyntax {
        ...
        case -2: throw new errors.X500Error("Invalid construction on BasicConstraintsSyntax");
        case -3: throw new errors.X500Error("Invalid tag number on BasicConstraintsSyntax");
        default: throw new errors.X500Error("Undefined error when validating BasicConstraintsSyntax tag");
        }
            ...
            if (element.tagClass === ASN1TagClass.universal) {
                if (element.tagNumber === ASN1UniversalType.boolean) {
                    if (element.construction !== ASN1Construction.primitive) {
                        throw new errors.X500Error("BasicConstraintsSyntax.ca was not primitively constructed");
                    }
                    if (index !== 0) {
                        throw new errors.X500Error("BasicConstraintsSyntax.ca was not the first element");
                    }
                    ca = element.boolean;
