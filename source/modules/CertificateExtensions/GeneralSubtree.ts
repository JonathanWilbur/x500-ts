...
class GeneralSubtree {
    ...
    public static fromElement (value: DERElement): GeneralSubtree {
        ...
        case -2: throw new errors.X500Error("Invalid construction on GeneralSubtree");
        case -3: throw new errors.X500Error("Invalid tag number on GeneralSubtree");
            ...
            if (element.tagClass === ASN1TagClass.context) {
                if (element.tagNumber === 0) { // minimum
                    if (element.construction !== ASN1Construction.primitive) {
                        throw new errors.X500Error("GeneralSubtree.minimum was not primitively constructed");
                    }
                    if (minimum) throw new errors.X500Error("GeneralSubtree.minimum already defined");
                    minimum = element.integer;
                    fixedPositionElementsEncountered++;
                } else if (element.tagNumber === 1) { // maximum
                    if (element.construction !== ASN1Construction.primitive) {
                        throw new errors.X500Error("GeneralSubtree.maximum was not primitively constructed");
                    }
                    if (maximum) throw new errors.X500Error("GeneralSubtree.maximum already defined");
