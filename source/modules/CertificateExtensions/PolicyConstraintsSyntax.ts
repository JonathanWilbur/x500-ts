...
class PolicyConstraintsSyntax {
    ...
    public static fromElement (value: DERElement): PolicyConstraintsSyntax {
        ...
        case -2: throw new errors.X500Error("Invalid construction on PolicyConstraintsSyntax");
        case -3: throw new errors.X500Error("Invalid tag number on PolicyConstraintsSyntax");
            ...
            if (element.tagClass === ASN1TagClass.context) {
                if (element.tagNumber === 0) { // requireExplicitPolicy
                    ...
                    if (requireExplicitPolicy) {
                        throw new errors.X500Error("PolicyConstraintsSyntax.requireExplicitPolicy already defined.");
                    }
                    ...
                    if (inhibitPolicyMapping) {
                        throw new errors.X500Error("PolicyConstraintsSyntax.inhibitPolicyMapping already defined.");
                    }
        ...
        if (!DERElement.isUniquelyTagged(policyConstraintsSyntaxElements.slice(fixedPositionElementsEncountered))) {
            throw new errors.X500Error("Elements of PolicyConstraintsSyntax were not uniquely tagged");
        }

        if (!DERElement.isInCanonicalOrder(policyConstraintsSyntaxElements.slice(fixedPositionElementsEncountered))) {
            throw new errors.X500Error("Extended elements of PolicyConstraintsSyntax were not in canonical order");
        }
