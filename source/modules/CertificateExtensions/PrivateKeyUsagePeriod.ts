...
class PrivateKeyUsagePeriod {
    ...
    public static fromElement (value: ASN1Element): PrivateKeyUsagePeriod {
        ...
        case -2: throw new errors.X500Error("Invalid construction on inner sequence of PrivateKeyUsagePeriod");
        case -3: throw new errors.X500Error("Invalid tag number on inner sequence of PrivateKeyUsagePeriod");
        default: {
            throw new errors.X500Error("Undefined error when validating inner sequence of PrivateKeyUsagePeriod tag");
        }
        }
        ...
        privateKeyUsagePeriodElements.forEach((element: ASN1Element) => {
            if (element.tagClass === ASN1TagClass.context) {
                if (element.tagNumber === 0) {
                    ...
                    if (notAfter) throw new errors.X500Error("PrivateKeyUsagePeriod.notAfter already defined");
        ...
        */
        if (!DERElement.isInCanonicalOrder(privateKeyUsagePeriodElements.slice(fixedPositionElementsEncountered))) {
            throw new errors.X500Error("Extended elements of PrivateKeyUsagePeriod were not in canonical order");
        }
