...
class EDIPartyName {
    ...
    public static fromElement (value: DERElement): EDIPartyName {
        ...
        case 0: break;
        case -1: throw new errors.X500Error("Invalid tag class on EDIPartyName");
        case -2: throw new errors.X500Error("Invalid construction on EDIPartyName");
        case -3: throw new errors.X500Error("Invalid tag number on EDIPartyName");
            ...
            if (element.tagClass === ASN1TagClass.context) {
                if (element.tagNumber === 0) { // nameAssigner
                    if (element.construction !== ASN1Construction.primitive) {
                        throw new errors.X500Error("EDIPartyName.nameAssigner was not primitively constructed");
                    }
                    if (index !== 0) throw new errors.X500Error("nameAssigner out of order in EDIPartyName");
                    ...
                    if (element.construction !== ASN1Construction.primitive) {
                        throw new errors.X500Error("EDIPartyName.partyName was not primitively constructed");
                    }
                    if (index > 1) {
                        throw new errors.X500Error("partyName out of order in EDIPartyName");
                    }
        ...
        if (!partyName) throw new errors.X500Error("EDIPartyName.partyName was not defined");
